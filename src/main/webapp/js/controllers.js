...
angular.module('bankApp')
    ...
    .controller('AccountsCtrl', function($scope, $http, $routeParams) {
        ...
        $scope.newTransaction = function() {
            $scope.addTransaction({amount: 0, type: 'DEPOSIT',account : {}});
        };
        ...
        $scope.findAccount = function(accountName,index){
        	 $http.get('/api/account/' + accountName)
             .success(function(account) {
            	 $scope.account.transactions.transactions[index].account = account;
             });
        }
